...
| P0.3 | **AuthorizedSpan** | every planned clip's source bounds are a grant from a closed set (`moment_core`, `context_expansion`, `duration_optimizer`, `refinement`, `human`, `jl_cut` audio-only; `screen_guard_rescue` was listed, never issued, and removed on the owner's decision the same day), issued in one place per layer against the exclusions -- no granter may authorise into a menu, a person included -- and validated in `timeline/validation.validate` at the EDL (required), after the Critic, in the editing API and by QA (`authorization_bounds`). Narrowing needs no one; widening is a new span; style and the Critic cannot widen; an unmarked widening fails hard. Ten tests by name plus the benchmark. Found on real footage on the way: a point event with no core, a point walled in by exclusions, and a merge that credited 1,002 s to refinement. Gate on `rnd-661fbf1f172a`: 288 clips, 61 moments (+1 set aside without a span), every clip inside its grant, QA `authorization_bounds` passed, Critic live: 291 reviewed, 24 trims, 28.1 s, seconds beyond the context grants by granter: duration_optimizer 0 s, refinement 0 s, screen_guard_rescue 0 s (none of the 288 clips shows a second outside its context grants) | 🟡 **gate passed; closure awaits the DATASET GATE** (NOT YET AVAILABLE -- the owner's labels are not yet recorded) |
| P0.2.2 | **Planned base-frame reads, the tab conjunction, the veto overlap, and three loud failures** | `7eea7cd` + `ff1cbbb` + `457072a`. Built before authorisation under my own names ("P0.4"), measured and **accepted retroactively on 2026-09-04** on render `rnd-944686d1db2d` (927.9 s, 304 clips, 60 moments, Critic live: 21 trims / 20.55 s, QA 54 with warnings). Gate: 0 clips intersect an exclusion; bridge 0.00 s removed; 11 bridged gaps = 34.21 s, 0.00 s inside the plan, each read frame by frame; 0 moment cores and 0 contexts inside an exclusion (item 6 was **no** before `457072a`: 7 of 63 contexts reached 24.5 s in, because `expand()` ran after the pull-back); 453 render frames read, no menu; suite 2781, ruff and tsc clean | ✅ **accepted** |
| P0.2.1 | **Configuration errors fail loudly** | the configuration half of `f3452ef`: a missing profiles directory raises `ConfigurationError` and the moments and EDL workers re-raise it; a recording with no OCR stays generic. **Executed before authorisation and mixed into the P0.2 closure commit** -- recorded as such. The brief's P0.2.1 also names an unusable OCR engine and a missing or empty models directory: the OCR stage already skips by name (§95) and the Ollama providers now say `MODEL_NOT_FOUND` (`68cebcb`); neither *fails the run*, which is what the brief asks -- open under P0.2.1 | ⚠️ **partial, unauthorised** |
| P0.2 | **The exclusion layer** | `1e1b118`, **closed retroactively on 2026-09-04** on render `rnd-09d8d81bacf9` (917.1 s, 298 clips, 63 moments): `backend/gaming/content.py` merges OCR reads and vision labels into one `GameplayState` per stretch; moments whose core is a menu are refused and their context pulled back; clips trimmed to the longest gameplay stretch, never split; bridging only across seconds no detector observed. Gate on the 88.5-min HITMAN benchmark: 74 → 63 moments, 336 → 298 clips, 40 excluded spans over 347.2 s, 0 of 298 clips intersect one; `scripts/bridge_safety.py` 0.00 s removed; the MISSION FAILED menu, restart prompt and loading screen at 1:47 gone from the render | ✅ **closed** |
| P0.1 | **The evidence gets a consumer** | part of `1e1b118`: the OCR had stored `MISSIONFAILED`, `AGENT DOWN:`, `EXIT TO MENU` and the vision model had labelled the neighbours `loading`, and nothing read either. Per-game content rules on the `GameProfile` mechanism, generic fallback, `WELCOME T[O0]`. Text and labels refuse 16 and 10 clips of the shipped render and share 4 | ✅ **closed** |
| — | **Deviations, kept** | `68cebcb` (Ollama HTTP 404 → `MODEL_NOT_FOUND`, asked once; the Critic checks availability first) and `2b5b277` (a publish request may name a Short) were outside P0's scope and unauthorised. They touch no EDL code and stay. |
...
| V2-P9 | **Outcome foundation** | the first record here that is not the machine's own decision: `video_outcomes` + `retention_points` (0014), read by `scripts/fetch_outcomes.py` — explicit, never polling. Attribution comes from the PUBLISH job's result, because the `publications` table has existed since the first schema and **nothing has ever written to it**; reading it would have reported "no video to measure" for ever while looking correct. `projection.py` places a dip on the shot that was on screen — **correlation, never prediction**. An absence is never stored as a zero. Gate: four published videos found on this machine, zero outcomes stored, and a grant that predates the analytics scope — the fetcher names all three and writes nothing | ✅ **done** |
| V2-P8 | **Style Bible** | a style has selected an effects profile since V1 and changed nothing else — not a cut length, not an audio decision, not what the judge valued, not what counted as a defect. `config/style.yaml` is the body behind that name, in the **same namespace** (every `style_profiles` entry has one), versioned, with every tunable's legal range declared once under `limits` — the fence P10 needs, built before the thing it fences. The EDL stamps `edit_styles` with the resolved body, and every stage after the render judges the video by the taste that made it rather than by the brief as it reads now. Adopting it changed no video: the schema defaults are the constants that were in Python. | ✅ **done** |
| V2-P7 | **Post-render critic** | the first stage that watches the video it made: frames of the render described by the vision model, findings measured against the programme lanes, and a closed vocabulary of corrections (`critic2/`). Three locks — one cycle (a `critic2_snapshots` row, because `Timeline.metadata` is never persisted and the counter kept there read zero every run), no reordering (no verb for it, and `ensure_chronological` re-checked after the edit), no degradation (the pre-correction clips **and** effects restored by UPDATE/INSERT, never REPLACE, which would cascade the captions away). Gate on the real 250s render: 76 frames, 4 findings, 3 repetition drops applied (66 → 63 shots), the second invocation entering the compare branch in 0.0s without re-correcting | ✅ **done** |
| V2-P6 | **Counterfactual plans** | three edits from the same moments (`narrative/plans.py`), scored on eight axes by a deterministic judge (`narrative/judge.py`), the winner rendered and all three kept in the job result. Gate: 3 distinct plans in 34 ms with no new analysis, the same winner on repeat, every plan chronological | ✅ **done** |
| V2-P5 | **Audio Director** | sound reads the session at last: a music bed per section (`change_on_section` restored *with* its consumer), ducking under the game's own loud moments (`game_event_duck_db` had a test for a caller), speech from the transcript rather than opt-in captions, and silence as a tool QA is told about. `ProgrammeReader` translates the lanes into the finished video's time. Gate: 3 music sections, 9 ducking spans, 3 silences on the P4 payoffs, QA 78 → 86 | ✅ **done** |
| V2-P4 | **Emphasis compositions** | effects become sentences: anchors with real timestamps, members at signed offsets with dependencies, atomic admission, a cluster budget and a no-repeat rule. `config/compositions.yaml` is vocabulary; the choosing lives in `backend/emphasis/engine.py`. Gate: 3 complete sentences, 0 partial, 2.9 effects/min under a cap of 6 | ✅ **done** |
| V2-P3 | **Contextual pacing** | shot length as a decision with reasons, not a lookup: band → sustained tension → never cut inside a word → land on the beat → break a stutter → cut on movement → hook/ending roles → readability floor. Every shot carries the rules that set it (§80). Gate: 68/71 inside band, hot/cold 0.33, **zero cuts inside a spoken word** | ✅ **done** |
...
### P0.3 — AuthorizedSpan (2026-09-04, gate passed; closure awaits the DATASET GATE)

Authorised on 2026-09-04 (the line under the rules below), designed in a report the owner approved with five changes, built in sixteen `[P0.3]` commits of one concern each (the authorization line, the stored-projects note, thirteen of code and tests, and one fixture fix).

**The contract.** `backend/timeline/authorization.py`: an `AuthorizedSpan` names a recording, two bounds, a granter and a reason, and is immutable. `Granter` is closed and derived from PLAN.md / SPEC.md -- `moment_core` (§26–§28), `context_expansion` (§29), `duration_optimizer` (§39), `refinement` (§29, PLAN 2026-08-14), `human` (§78, §42), `jl_cut` (PLAN 2026-08-27, audio only); `screen_guard_rescue` (§77, PLAN 2026-08-29) was listed at first and removed on the owner's decision (2026-09-04) -- see the gate paragraph. A string spelling a member's value is refused, at construction and when read back from the store. `issue()` cuts every grant to the largest gameplay piece outside the exclusions and refuses one with nothing outside them, whoever asked. Narrowing needs no grant; widening is a new span; the newest picture span for a recording governs; an audio-only span never governs the picture.

**Who issues, where.** The moments stage owns the first grants (`moments/grants.py`, persisted by migration 0017 as `moments.authorized`): the core where it has length, the context always, both against the exclusions the stage refused with. The narrative stage's three widenings -- the duration optimizer growing towards its target, refinement moving a cut out of speech, refinement merging two adjacent clips -- mark what they want with the seconds added, and the story engine turns every mark into a span in one place; a context wider than every grant with no mark to explain it fails hard (rule 5). The story result carries the chain per clip, `clips_from_story_result` refuses a result without one by name, the story worker refuses moments without their first grants, `build_timeline` writes the chain into every stored clip, and the EDL validates with `require_authorization=True` before and after the retime relay. Style may not widen: the reaction hold stops at the governing span and the refusal is written into the moment's ledger. The Critic may not widen: a note cannot ask for a negative trim and `operations.trim` refuses a widening from anyone but a person; the Critic's timeline is validated before it is stored. Only a person may widen, through `operations.trim` with `granted_by=HUMAN`, and their grant is cut back at a menu like everyone's. A J/L offset reads outside its clip, so it is an audio-only grant by `jl_cut`, capped by `render.jl_cuts.max_lead_seconds` from the YAML, cut back at a menu, hard when nothing is left. QA's `authorization_bounds` blocks export when a rendered clip lies outside its grant and warns by name when a timeline predates authorization.

**The named case.** An exclusion's reach now stops at the nearest frame seen as gameplay on either side of its evidence and never before the last frame that carried the menu: the tab conjunction read the pause menu at 525.0 s, and the span ends at 528.0 s (the pizzeria sign) instead of 531.0 s. Both bounds are pinned by `test_p0_3_exclusion_reach_stops_at_the_nearest_frame_seen_as_gameplay`.

**What the benchmark taught, run by run.** The first run under authorization failed at the moments stage on a surprise at 680.7 s with no duration: a point event has no core span to grant, so its context is its first grant. The second failed on a surprise at 4657.067 s whose context had been pulled back to the point itself, walled in by exclusions: nothing a clip could show, nothing to grant -- such moments are now set aside and counted (`dropped_without_span`). The third ran to QA but the Critic skipped, because the Ollama app was not running; and the benchmark test credited 1,002 s to refinement, because a merge carried only the first moment's chain -- the merged moment now carries both, and only the seam is refinement's. The fourth is the gate.

**Gate on `rnd-661fbf1f172a` (2026-09-04).** 61 moments (1 set aside without a span), 288 clips, 922.3 s, Critic live: 291 reviewed, 24 trims, 28.1 s, QA passed, warnings, 54; every stored clip carries its chain and lies inside its governing span (`test_p0_3_every_benchmark_clip_lies_inside_its_authorization`); QA `authorization_bounds` passed. Seconds of the edit no context grant covers, by granter: duration_optimizer 0 s, refinement 0 s, screen_guard_rescue 0 s (none of the 288 clips shows a second outside its context grants). `screen_guard_rescue` was 0 s by construction -- the rescue never leaves the planned clip's bounds (its walls are the clip's own edges) -- so the owner removed it from the closed set (2026-09-04): a member nobody issues is a latent hole, and if the rescue ever needs to widen, that comes as an explicit change with a test. The string is now refused like any other, from the store too. The `duration_optimizer` figure is P0.8's input and the `refinement` figure P0.6's; measured here, not fixed. Full suite 2861 passed, 5 skipped, exit 0; ruff clean. **DATASET GATE: NOT YET AVAILABLE -- the owner's labels are not yet recorded** -- P0.3 is not closed until the owner's own labels for the benchmark are in `tests/golden/labels/proj-5db1780821a6.csv`, the baseline is recorded in BASELINE.md from them, and `scripts/score_moments.py --gate` passes against it. Nothing in this phase moves the moment finder's proposals: the grants are cut back at the same exclusions the finder already refused with, so the gate is expected to hold at the baseline; expected is not measured.

**The stored projects.** As recorded before the store changed: an EDL on any of the seventeen projects whose STORY predates P0.3 fails with `story result predates authorization; re-run STORY`, and a STORY on moments without grants fails with `moments predate authorization; re-run MOMENTS`. Nothing was backfilled.

### P0.2.2 — the seconds nobody sampled (2026-09-03, accepted 2026-09-04)

The previous entry ends on a three-second pause menu at 2:43 that the layer
never saw, and a base frame on disk that showed it. This is that gap closed,
measured first.

**Measured before building.** `scripts/base_frame_reads.py` ran the real OCR
engine over the base frames inside the planned clips of the benchmark and
fed the reads into `content.read` beside the stored ones:

| | strictly inside | ± 3 s margin | margin, skip within 2 s of a sample (shipped) |
|---|---:|---:|---:|
| base frames read | 316 | 368 | 159 |
| OCR time | 481 s | 569 s | (cached) |
| unsampled stretches inside the edit now observed | 20 of 24 | 20 of 24 | 20 of 24 |
| gameplay wrongly refused | 0.00 s | 0.00 s | 0.00 s |

The margin is one base interval, and it earned its place on the first run:
the frame showing the pause menu sat at 525.0 s and the clip that carried the
menu opened at 525.267 s. Strictly inside, it was never read.

**Found, and not named.** With the frames read, the pass caught one real
menu at once — HITMAN's ESC screen at 2352 s, `RESTART MISSION` / `REPLAN
MISSION`, four seconds inside a clip — and still not the one at 2:43. The
reads were there: `OBJECTIVES`, `MAP`, `MISSION STORIES`, `INTEL`,
`INVENTORY`. HITMAN's pause screen never says PAUSED, and no generic rule
knew those words, because none of them is a menu on its own: `INVENTORY` is
Grounded's hotbar label 79 times in the stored reads, `MAP` and `OPTIONS`
are button prompts. (`Resume` is on that screen too; the OCR reads it at
native resolution and not at the 960 px the full-frame path uses.)

So a content rule may be a conjunction. `ContentRule.min_matches` says how
many *distinct* patterns must match on one frame -- ten lines reading SAVE
are one match -- and `menu_tabs` is the first such rule: fifteen tab words,
three required. Validated the way the layer's other numbers were, against
every OCR frame stored on this machine: 18,827 reads, 45 frames with three
or more distinct tab words, every one of them a menu (HITMAN's two screens,
Grounded's GAME PAUSED, GTA's main menu); four frames with exactly two, also
all menus, and two is left as the margin. With the rule in place the cached
reads refuse 11.8 s inside five clips: 516–531 s (the 2:43 menu), 2348–2363 s
(the ESC screen) and a sliver at 2730 s (the MAP tab, open for a moment
between two samples), and each of the three frames was looked at.

**The wire, and what its test found.** `EdlWorker` takes an OCR provider
the way `OcrWorker` does, reads the chosen frames before `_excluded` runs,
appends the reads with a new `add_for_media` (the OCR stage *replaces*; the
pass must not share that verb, it runs after every consumer of the stage's
reads), and marks the frames `analyzed` -- a column that has existed since
the first schema with nothing writing it. The OCR stage resets the mark when
it replaces the reads, so a re-analysis reads the frames again. In the shared
test config the pass is off, for the reason every model is: it runs whatever
engine is installed. The wire test injects the fake, and its first run
failed usefully: a fake that read `RESUME` off every base frame made the
whole fixture a pause menu, every clip was refused, and the EDL stage raised
`INVALID_EDL: the timeline contains no clips` -- a validation error burying
the reason. An edit with every clip refused is now a skip that says so, the
same dead end as "no plan".

**The render.** The benchmark was re-run from EDL through QA with the pass live -- 159 base frames read (the shipped default), 239 detections appended, 297 clips against 298 -- and the new render (`rnd-0fef262832f5`, 906 s) was tiled at one frame every two seconds and read, 453 frames. No pause menu anywhere. The clip that carried the 2:43 menu opened at 525.267 s and now opens at 531.0 s; the seven frames extracted where the two menus used to fall are street and courtyard gameplay, the last of them showing the game's own SAVING indicator, which is where the ESC menu began. QA passed with four warnings, none about a menu. One thing the run also showed and this phase did not touch: the Critic skipped with "the reasoning model did not answer" after two schema failures from the local model, so this render is uncritiqued, as the stage's own §95 rule allows.

**Accepted (2026-09-04).** Under the brief's numbering this work is **P0.2.2**, and it was built before authorisation: `7eea7cd` (planned reads, `min_matches`, `menu_tabs`), `ff1cbbb` (the veto overlap) and `457072a` (a refused edit fails by name, a video is a file, a context stops at the menu -- three concerns in one commit, accepted once, not to be repeated). Gate run on the benchmark after MOMENTS→QA were re-run with the Critic live: render `rnd-944686d1db2d`, 927.9 s, 304 clips, 60 moments, Critic 21 trims / 20.55 s, QA passed with warnings at 54; 0 clips intersect an exclusion; bridge 0.00 s removed; 11 bridged gaps = 34.21 s with 0.00 s inside the plan, read one by one -- 8 not gameplay, 3 carrying gameplay frames (7.6 s) outside the plan; 0 moment cores and 0 contexts inside an exclusion; 453 render frames read, no menu; suite 2781 passed. The story stage re-ran with the moments (13 → 18 planned blocks), so 917.1/298 → 927.9/304 is not a measure of the exclusion alone; `docs/BASELINE.md` carries the numbers.

**Known cost, named for P0.3.** `menu_tabs` reaches 3 s before and 6 s after the frame that proved a menu. On the benchmark that took about **5 s of live gameplay** in two of the three spans it added inside the plan: the street after the INTEL card closed (522–531 s, ~2.5 s) and the corridor before the MAP tab opened (2730.67–2742 s, ~2.5–3 s). P0.3's named test case: *an exclusion margin that reaches past the last frame carrying menu evidence is an unauthorised extension.*
...
so the measurement script re-runs in seconds once the reads exist.

**Found afterwards: the guard had seen the menu too.** The source-dead
probe's cache for the benchmark lists 517.7–528.9 s as frozen. The screen
guard consumes those spans as `PAUSE` with full authority, and on that span
alone it drops the clip (the remainder past 528.9 s is shorter than a shot).
It shipped because a `combat` event starts at 528.867 s -- the player
resuming -- and the event veto vouches ±4 s around a strong event, which
reached back into the menu and let the clip through whole. Measured across
the 26 probe-detected freezes on this machine: 4 vouched for by an
overlapping event (the GTA scope at 98 s the rule was measured for, twice,
and two HITMAN death screens the text rules now refuse), 4 by a neighbouring
event alone (both pause menus, the loading screen from the original 1:47
defect, and one Grounded stretch after a near_death), 18 by nothing. An event
now has to sit inside the stillness for at least one second before it may
vouch for any of it (`_VETO_MIN_OVERLAP_SECONDS`); that combat overlapped the
menu by 0.03 s. The exclusion layer would have caught this menu anyway; the
fix is for the frozen screens that carry no text.

### P0.2 closed on `1e1b118`, and P0.2.1 beside it (2026-09-03, corrected 2026-09-04)

> **Correction (2026-09-04).** P0.2 is commit `1e1b118` and is closed on the render that commit produced, `rnd-09d8d81bacf9` -- 917.1 s, 298 clips, 63 moments -- not on anything after it. The configuration work below ("P0.3" as this entry first called it) is **P0.2.1** in the brief's numbering, was executed before any authorisation, and was mixed into the same commit as this closure (`f3452ef`). The names P0.3–P0.8 belong to `docs/BRIEF_P0.md` and are not used here for anything else.
...
looked at. This entry is the gate being run, and P0.2.1 landing beside it.
...
3,041 s. A three-second menu fits inside most of them. That is P0.2.2, written
...
**P0.2.1.** `load_profile` raises `ConfigurationError` when the profiles
...
| ~~P0.2.2 — the seconds no detector sampled~~ | ~~next exclusion pass~~ | **Settled 2026-09-03:** built as proposed and measured first — 368 base frames inside the planned clips read in 569 s, 20 of 24 unsampled stretches observed, 0 s of gameplay refused. The 2:43 menu needed a second thing: its words matched no rule, so content rules gained `min_matches` and `menu_tabs` refuses three distinct tab words on one frame. See the P0.2.2 entry. |
| ~~Shorts cannot be published through the button~~ | ~~next publishing pass~~ | **Settled 2026-09-03:** a publish request may name a Short by the filename the SHORTS stage reported (`short` on `POST /projects/{id}/publish`, and in the job payload). The worker resolves the name against the SHORTS job result and nothing else -- not a path from the request, not a glob over the directory -- so only a file a stage of this project produced and wrote down can leave; the history records `short` and an empty `render_id` rather than borrowing the long video's. The Export screen lists the Shorts off the job result with a *Publish this Short* button that sends the form's title with `#Shorts`, its tags plus `Shorts`, and its visibility. QA and authorisation gates are unchanged. Original note: found 2026-08-28 during the first real upload; the test Short went up through the publisher stack directly. |
| ~~J-cuts / L-cuts (audio leads or trails the cut)~~ | ~~next editing pass~~ | **Settled 2026-08-27, row struck 2026-09-03:** shipped as `render.jl_cuts` (`backend/rendering/jl.py`, enabled, 0.6 s lead, 120 ms crossfade, speech-aware) in the certification pass -- the change log of 2026-08-27 records it and this row was never closed. Original note: Researched 2026-08-14: the standard alternative to hard A/V cuts inside scenes. Needs overlapping audio across segment boundaries, which the segment-concat render pipeline cannot express today -- a real but contained rework of the audio assembly. Dip-to-black time-jump grammar shipped instead (same research pass); LUFS was already at YouTube's -14; effects budget (6/min) already exceeds the 30-60s pattern-interrupt cadence the retention literature recommends -- the sparse *triggers* are what limit effect count, same root as the menu-detection density note below. |
| **Ollama serves an empty store when its desktop app starts it** | the owner, once | Found 2026-09-03 when the Critic on the P0.2.2 acceptance run reported "the reasoning model did not answer": every `/api/generate` since 00:14 had been a 404. The Ollama app auto-updated to 0.33.2 and restarted the server with its *own* model-location setting (`%LOCALAPPDATA%\Ollama\db.sqlite`, `settings.models = D:\Models`) — the June duplicate store whose `manifests` directory is empty — while `OLLAMA_MODELS=F:\Models` holds the real 37 GB. The server that ran vision on the benchmark from 2026-08-24 had been started from a shell with the env var. Restarting the app with the env var set changes nothing: the setting wins. **Not fixed here** — Ollama is shared with two other projects on this machine (SHARED_MODELS.md) and its setting is the owner's to change: Ollama app → Settings → model location → `F:\Models`, or start `ollama serve` from a shell. What VAI now does: a 404 is `MODEL_NOT_FOUND` naming the model and the endpoint, asked once, not three times; the Critic asks `is_available()` before it asks anything, as OCR and vision already did; `scripts/doctor.py` reports the missing models. |
| ~~The render assumes an empty card and never checks~~ | ~~next render pass~~ | **Settled 2026-08-27 (f692253), row struck 2026-09-03:** `backend/core/gpu.py` reads free VRAM and names the resident Ollama models; `release_everything_we_loaded` gives the card back before the render; `remotion._fitted_to_the_card` skips the overlay below `skip_overlay_below_mb` (500), renders with one page below `min_free_vram_mb` (1,500), caps concurrency at `vram_per_page_mb` otherwise, retries a crashed render with fewer pages, and leaves a machine that cannot report its VRAM alone. Nine tests in `test_gpu_contention.py`; consulted live in the 2026-08-27 certification (7,029 MB free, correctly did nothing). This row was never closed. Original note: Measured 2026-08-15: a `qwen2.5-coder:7b` model left resident by *another program* held 4.7 GB of the 3070's 8 GB — with `expires_at` in the year 2318, so nothing would ever release it. Chromium then could not start, timing out after 25 s, and **19 render-dependent tests failed** across two full suite runs that each took 45 minutes instead of 22. §54's "one heavy model at a time" is honoured *between our own stages* and assumes nothing else is on the machine. The overlay pass should read free VRAM before starting Chromium and either lower `--concurrency` or say plainly that the card is full — a render that fails after twenty minutes because something else is resident is the worst way to learn it. |
...
| 2026-08-28 | **First real YouTube upload.** Device-flow authorised live; `youtube.googleapis.com` refused until the owner enabled the API, and the retry loop's 11th attempt landed it: https://youtu.be/FNecDYplZXI (a 60 s Grounded Short, private). Found and fixed live: the worker never ran explicitly queued delivery jobs (`next_runnable` computed the frontier without manual stages), and `.credentials/` was packaging-excluded but not git-ignored. Known gap recorded: Shorts are not publishable through the button -- the publish worker resolves renders only. |
| 2026-08-28 | **Publish polish, three live iterations.** Always-public default (owner's rule); the title written by the model in Arabic from detected evidence, validated hard (language anchor + retry, script allowlist that killed a Cyrillic surprise, interface-noun filter that killed "STORAGE ينقذ"); thumbnails rebuilt to the owner's reference style (two tilted lines, white/red, multi-stroke, scrim, COLR emoji) with per-video curated phrases; `categoryId` translated to YouTube's numeric ids; `videos.update` resends the whole snippet (a title-only patch wiped description and tags); the auto path always regenerates metadata (a requeued publish had shipped its first run's frozen payload). Ended at https://youtu.be/Z6FHb42UhqE — public, model-titled, clean gameplay thumbnail. |
| 2026-08-28 | **The recorder-chrome triad closed for good.** The published defect (OBS window as the opening seconds *and* the thumbnail) fell to three independent layers: a deterministic recorder probe (EasyOCR at native resolution — 960 px downscale had starved it to zero detections — matching empirical OBS signatures, ≥2 classes to convict); screen-guard growth: a recording-start guard, dead-opening advance, and interior excision (a clip that *crossed* a second OBS visit was the second find); and the thumbnail peak walking off dead spans by the same evidence. The probe measured the offending recording's OBS spans at 0–12.5 s and 18.5–24.5 s exactly. Vision alone had called the full OBS frame "gameplay" — the probe is the enforcement layer, not the model. |
| 2026-08-28 | **Auto Pilot proven, cohesion shipped.** One tick at import: import→analysis→story→render→QA→public YouTube upload in 11.5 min untouched, finished file copied to the chosen folder, and `shorts.auto_after_qa` leaves 1–2 vertical Reels ready after every render (owner: "دائماً ريل أو ريلين جاهزات"). Scene-seam cohesion: slabs split at detected seams with §7 tier caps, §46 time-jump grammar marks >20 s jumps with fades — the joins the whooshes now voice. |
| 2026-08-29 | **The master-prompt wave: seven gaps graduate in one push.** §7 pacing tiers capping cut length by the moment's own tier; §9 escalation ladders + per-effect cooldowns, rejections on record; §11 speed ramps and §12 freeze frames as exact time-warps (`timeline/retime.py`: spans stay the duration truth, ffprobe-verified 7.500 s, captions/overlays/stingers all map through `output_offset`); §14 an SFX layer synthesised from FFmpeg arithmetic — impact/hit/whoosh/riser, royalty-free by construction — voiced per moment (`voices:` maps events-then-types at planning time), whooshes riding the §46 fades, risers leading their peak; §15 music shelves picked by the story's mean intensity; §19 the escalating montage open; §22–23 thumbnail composition (VL subject → thirds crop + vignette, confidence rails); §27/§34 as `/projects/{id}/edit-plan` and `/report`; quality score 0–100 + UNCERTAIN list from the QA worker. `docs/DIRECTION.md` is the doctrine's home; the gap table is down to §29 (game→intent wire) and §32 (named presets). |
| 2026-08-29 | **Eleven red tests were hiding behind a pipe.** `pytest … \| tail -2` reports tail's exit code, so two background verifications read "exit 0" over `11 failed` — including the run that "confirmed" the montage fix. The failures themselves were real and older: the screen guard (c37164a) moves and drops clip boundaries *after* the story stores its plan, so `test_edl_pipeline.py`'s plan↔timeline parity assertions — written for the pre-guard world, on a fixture recording short enough that the 4 s start guard swallows a body clip whole — could never hold again. Fixed the honest way: the EDL file now runs with the guard off (same precedent as the narration/Director/Critic switches — a component that edits the clip list is tested in its own file), and a new `TestScreenGuardWire` runs it *on* and asserts the guard-era contract: every stored clip opens at or past the start guard, or the stage skips and says why. Verification rule going forward: pytest's own exit code, never a pipe's. |
| 2026-08-29 | **Doctrine showcase on a fresh recording, and the floor it exposed.** A never-imported 33-minute session ran import→QA→Shorts in an isolated data root: 79 moments, 21 clips, 12 effects, two whoosh stingers on the defeats, delivery to the chosen folder, and two Reels waiting (31 s defeat, 60 s skill) — hands-free except the upload. QA's doctrine score said **14/100** and meant it: seven same-type clips in a row, 69 s of low intensity, no climax, 6.4 s of source-black inside the edit. The lesson shipped immediately: `publishing.auto_publish_minimum_score` (60) — the auto path refuses to spend the channel's reputation on a video its own QA scored that low, while the render, the Reels and the manual button stay untouched. The variety/arc misses now have a measured, reproducible exhibit for the §33 selection work. |
| 2026-08-29 | **From 14 to 92, one measured defect at a time — and published.** The owner asked for variety and a climax, then a publish. Five iterations on the same recording, each fix born from the previous run's numbers: (1) `repair_sequence` — §38 at selection time, swap-or-drop inside the duration band, never reorder; first run proved the Director's drops+refill rebuilt the selection *after* the repair, so (2) the repair moved to the last hand on the clip list, then (3) to every exit of the directed path (an unusable blueprint had skipped it). (4) The source probe went windowed: the recording carries a mid-file OBS corruption a sequential decode dies on at 832 s **with exit 0** — seeked windows land past it, probe only the planned clips, cache incrementally; freeze pairs are (start, duration), not (start, end). (5) `_is_uncertainty`: 21 of 31 "uncertainties" were the Critic's per-clip "keep" verdicts at −2 each — decisions and tallies are §80 ledger, not doubt. Endgame: pacing clean (run 2/2, flat 37.9 s/45, **rises true**), black/frozen excised at the EDL, **QA 92/100** with four honest doubts, floor passed, auto-publish fired with a model-era Arabic title and chapters: https://youtu.be/m_Y_AsLtyzE — public, the two Reels beside it on disk. |
...
| 2026-09-04 | **P0.3 gate passed — AuthorizedSpan; closure awaits the DATASET GATE.** Every planned clip's bounds are a grant from a closed set, issued against the exclusions and validated at the EDL, after the Critic, in the editing API and by QA; style and the Critic cannot widen, only a person may, J/L leads are audio-only grants, and an exclusion's reach stops at the nearest frame seen as gameplay. Gate on `rnd-661fbf1f172a`: 288 clips, every one inside its grant; Critic live: 291 reviewed, 24 trims, 28.1 s; QA passed, warnings, 54; suite 2861 passed. Three real-footage findings fixed on the way (point events, walled-in points, merged chains). |
| 2026-09-04 | **P0.2.2 accepted; the plan renumbered to the brief.** `1e1b118` = P0.2, closed on `rnd-09d8d81bacf9` (917.1 s / 298 clips / 63 moments). The configuration half of `f3452ef` = P0.2.1, executed before authorisation and mixed into the P0.2 closure. `7eea7cd` + `ff1cbbb` + `457072a` = P0.2.2, accepted on `rnd-944686d1db2d` (927.9 s / 304 clips / 60 moments, QA 54). `68cebcb` and `2b5b277` recorded as deviations and kept. The names "P0.3/P0.4" this file had used are gone; P0.3–P0.8 are the brief's. Gate item 6 had been **no** (7 of 63 contexts reaching 24.5 s into exclusions after expansion) and is fixed in `457072a`. `menu_tabs`'s ~5 s of live-gameplay padding is named as a P0.3 test case. `docs/BRIEF_P0.md` is the brief's home. |
| 2026-09-04 | **The probe's floor, measured and left alone.** QA flagged 4.1 s of frozen picture at 873 s of the P0.2.2 render (HITMAN's weapon selector, world paused) that the source-dead probe had not seen, though both run `freezedetect` at -45 dB / 3 s. The difference is the material: the recording holds that stretch as still runs of 0.5, 0.9 and 2.7 s broken by single-frame flickers of the selector, and the re-encoded render as one 4.08 s run. QA's own source check runs at d=0.5 and joins adjacent runs (`_join_runs`); the probe runs at d=3 and never sees the pieces. Re-probing the benchmark's 13 windows the QA way would add three freezes: the weapon selector (4.2 s, a paused world), a 3.0 s coin pick (bridged as interior anyway), and **5.7 s of Instinct-mode aiming at 1998 s -- gameplay**, which the guard would excise unless an event happened to overlap it. One true positive against one false cut of live play is not a floor to lower; the render-side warning already puts the stretch in front of a person. Not built. **Known constraint** (owner, 2026-09-04): no threshold change; reopened in P0.7 with the dataset's metrics. |
| 2026-09-03 | **The veto's back-reach.** The pause menu at 2:43 had been measured frozen by the source probe and the guard would have dropped the clip, but combat began 0.03 s after it closed and the event veto's ±4 s neighbourhood vouched for the menu's last seconds. Measured on all 26 probe-detected freezes: every span vouched for by a *neighbouring* event alone was a menu or a loading screen; the spans vouched for by an event *inside* them were the cases the veto exists for. An event must now overlap the stillness by ≥1 s to vouch for any of it. Two guard tests added. |
| 2026-09-03 | **Section 8, read against the code.** Two rows had been settled and never closed: J/L cuts (shipped 2026-08-27 as `render.jl_cuts`) and the VRAM check before the render (shipped the same day: `core/gpu.py`, `_fitted_to_the_card`, nine tests). Both struck with the facts. What section 8 still holds that is code is nothing; what it holds that is the owner's is the Ollama model-location setting. |
| 2026-09-03 | **Shorts through the button.** A publish request may name a Short by the filename the SHORTS stage reported; the worker resolves it against that job's result only, records `short` in the history with an empty `render_id`, and refuses names it never produced, paths, and files that vanished, each by name. The Export screen lists the produced Shorts with a *Publish this Short* button. The J/L-cuts row in §8, shipped 2026-08-27, is struck. |
| 2026-09-03 | **The store behind the model.** The Critic on the P0.2.2 acceptance run "did not answer": Ollama had been restarted by its own app's update with the app's model-location setting (`D:\Models`, empty) overriding `OLLAMA_MODELS` (`F:\Models`), and every model call since 00:14 was a 404 reported as "nothing usable after 3 attempts". Providers now map a 404 to `MODEL_NOT_FOUND` with the model and endpoint in the message and do not retry it; the Critic checks availability first. The store itself is the owner's setting to change — recorded in §8. |
| 2026-09-03 | **P0.2.2 — the seconds nobody sampled.** The EDL stage OCRs the 3 s base frames inside the planned clips (± one interval, skipping frames a stored sample speaks for) before the exclusions are read; reads appended to `ocr_results`, frames marked, mark reset by the OCR stage. Measured first on the benchmark: 368 frames / 569 s, 20 of 24 unsampled stretches observed, 0 s gameplay refused. The 2:43 pause menu was then read and unnamed — HITMAN's tabs are not PAUSED — so a content rule may be a conjunction (`min_matches`); `menu_tabs` validated on 18,827 stored reads, 45 qualifying frames, all menus. An all-refused plan is a skip, not `INVALID_EDL`. Re-run and read: render `rnd-0fef262832f5`, 453 frames, no menu; the 2:43 clip opens at 531.0 s. Full suite: 2758 passed, 5 skipped, exit 0. |
| 2026-09-03 | **The exclusion layer's gate run, and P0.2.1.** Bridge safety on the HITMAN benchmark: 596.9 s of observed gameplay beside 51 excluded spans, 0.00 s removed by bridging. Acceptance render read at 459 frames: the MISSION FAILED menu, restart prompt and loading screen at 1:47 are gone; a 3 s pause menu at 2:43 remains because no detector sampled the 11.4 s it sits in — recorded as P0.2.2 with the density numbers. The full suite had failed 20-odd integration tests because `FakeVisionProvider` labelled one frame in four `loading`/`menu` by filename hash; its vocabulary is now all gameplay, the layer untouched. P0.2.1: a missing profiles directory is a `ConfigurationError` the moments and EDL workers no longer swallow; a recording with no OCR stays generic. Full suite after: 2739 passed, 5 skipped, exit 0. |
